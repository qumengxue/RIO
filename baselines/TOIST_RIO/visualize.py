...
def draw_box_mask(img_ori, targets, res, out_path_i, captions):
    score_thresh = 0.95
    # score_thresh = 0.9
    SAVE_BBOX = True
    SAVE_MASK = False
    SAVE_BBOX_MASK = False
    alpha = 0.5
    ...
    for i in range(len(img_ori)):
        ...
        if SAVE_BBOX:
            ...
            box_out_file_path = out_path_i+'/'+str(image_id)+'_bbox_both_'+captions[i]+'.png'
            ...
            # cv2.imwrite('/opt/data/private/qmx/code/task_refer/data_select/static/hard/image/{}.png'.format(str(image_id).zfill(8)), img_array_both)
            # with open('/opt/data/private/qmx/code/task_refer/data_select/static/hard/label/{}.txt'.format(str(image_id).zfill(8)),'w') as f:
            #     f.write(captions[i])
...
def main(args):
    ...
    for i, item in enumerate(val_tuples):
        ...
        for j, batch_dict in enumerate(item.dataloader):
            ...
            draw_box_mask(batch_dict["img_ori"], targets, res, out_path_i, captions)
            print(j, image_id)
