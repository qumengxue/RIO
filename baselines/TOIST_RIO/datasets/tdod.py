...
id2name = {2: 'bicycle', 3: 'car', 4: 'motorcycle', 5: 'airplane',
...
class TdodDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        if 'refcoco_task' not in dataset_name:
        ...
        if not self.args.distillation: # plain TOIST
            if self.args.verb_noun_input:
                ...
                if self.image_set == 'train':
                    caption = target_ori[0]['descrip']
                else:
                    # caption = target_ori[0]['descrip'].replace(id2name[target_ori[0]['COCO_category_id']], 'something').lower()
                    caption = target_ori[0]['descrip']
                ...
                if 'refcoco_task' not in dataset_name:
                    ...
                    caption_sth = 'something ' + target_ori[0]['descrip'].replace(' the '+id2name[target_ori[0]['COCO_category_id']], '').lower()
...
class TOISTConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target, gt_obj):
        ...
        if 'refcoco_task' not in target["dataset_name"]:
            task_caption = TasksAbbreviation[int(dataset_name.split('_')[1])]
        ...
        if gt_obj == 1: # noun
            ...
            for obj in anno:
                ...
                if begin_idx != -1:
                    ...
                    noun_tokens_positive.append([[0, len(caption)]])
        else: # pronoun
            tokens_positive = []
            for obj in anno:
                # tokens_positive.append([[0, len(caption)]])
...
def build(dataset_file, image_set, args, tokenizer):
    if image_set == 'train':
        ...
        ann_file = Path(args.refexp_ann_path) / ('refcoco_task_train.json')
        # ann_file = Path(args.refexp_ann_path) / ('task_%s_train.json' % dataset_file.split('_')[-1])
        
    else:
        img_dir = Path(args.coco_path) / "val2014"
        if not args.eval:
            ann_file = Path(args.refexp_ann_path) / ('refcoco_task_500.json')
        else:
            ann_file = Path(args.refexp_ann_path) / (f'refcoco_task_test_{args.refexp_test_set}.json')
        # ann_file = Path(args.refexp_ann_path) / (f'refcoco_task_test_{args.refexp_test_set}.json')
