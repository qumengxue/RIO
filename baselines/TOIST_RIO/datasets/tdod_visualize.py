...
with open('data/id2name.json', 'r') as id2name_f:
...
class TdodDetection(torchvision.datasets.CocoDetection):
    ...
    def __getitem__(self, idx):
        ...
        target = {"image_id": image_id, "annotations": target_ori}
        target["dataset_name"] = dataset_name
        target["tokens_positive"] = [0,9]
        ...
        if "tokens_positive_eval" in coco_img and not self.is_train:
            tokenized = self.prepare.tokenizer(target["caption"], return_tensors="pt")
...
class TOISTConvertCocoPolysToMask(object):
    ...
    def __call__(self, image, target):
        ...
        caption = 'something ' + target["annotations"][0]['descrip'].replace(' the '+id2name[target["annotations"][0]['COCO_category_id']], '').lower()
...
def build(dataset_file, image_set, args, tokenizer):
    if image_set == 'train':
        ...
        ann_file = Path(args.refexp_ann_path) / ('refcoco_task_ori_sent_no_food.json')
        # ann_file = Path(args.refexp_ann_path) / ('task_%s_train.json' % dataset_file.split('_')[-1])
        
    else:
        img_dir = Path(args.coco_path) / "val2014"
        ann_file = Path(args.refexp_ann_path) / ('refcoco_task_val_selected.json')
        # ann_file = Path(args.refexp_ann_path) / ('refcoco_task_val_hard.json')
