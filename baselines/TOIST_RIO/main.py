...
from datasets.coco_eval import TDODCocoEvaluator, RefTaskEvaluator
...
def main(args):
    ...
    model.to(device)
    # model.to(torch.cuda.get_currect_device())
    ...
    if args.distillation:
        ...
        if args.distributed:
            model_noun = torch.nn.parallel.DistributedDataParallel(model_noun, device_ids=[args.gpu], broadcast_buffers=False, find_unused_parameters=True)
    ...
    def build_evaluator_list(base_ds, dataset_name):
        ...
        if dataset_name[:4] == "tdod":
            evaluator_list = [RefTaskEvaluator(base_ds, tuple(iou_types)), TDODCocoEvaluator(base_ds, tuple(iou_types), useCats=True)]
